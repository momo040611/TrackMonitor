...
import { createTracker, createBehaviorPlugin, createErrorPlugin, createPerformancePlugin } from '../../../../track-sdk/src';
...
const UserTracking: React.FC<UserTrackingProps> = () => {
  ...
  const detectPlatform = (): string => {
    if (typeof (window as any).wx !== 'undefined' && (window as any).wx.miniProgram) {
      ...
      return 'nodejs';
    } else if (typeof (window as any).process === 'object' && (window as any).process.type === 'renderer') {
      return 'electron';
    } else {
      return 'web';
    }
      ...
      <div style={{ 
        ...
        <div style={{ 
          ...
          <div style={{
            ...
            <p style={{ margin: '0', fontSize: '24px', fontWeight: 'bold' }}>{activePlatform === 'web' ? '网页' : activePlatform === 'mini-program' ? '小程序' : activePlatform === 'nodejs' ? 'Node.js' : 'Electron'}</p>
          ...
          <div style={{
            ...
            <p style={{ margin: '0', fontSize: '24px', fontWeight: 'bold' }}>{trackingData.length}</p>
        ...
        <div style={{ marginBottom: '24px' }}>
          <div style={{ 
            ...
            {['web', 'mini-program', 'nodejs', 'electron'].map((platform) => (
              ...
              >
                {platform === 'web' ? '网页' : 
                 platform === 'mini-program' ? '小程序' : 
                 platform === 'nodejs' ? 'Node.js' : 'Electron'}
              </button>
            ))}
          </div>
          ...
          <div style={{ 
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead style={{ backgroundColor: '#fafafa' }}>
                <tr>
                  <th style={{ padding: '8px', borderBottom: '1px solid #e8e8e8', textAlign: 'left' }}>类型</th>
                  <th style={{ padding: '8px', borderBottom: '1px solid #e8e8e8', textAlign: 'left' }}>时间戳</th>
                  <th style={{ padding: '8px', borderBottom: '1px solid #e8e8e8', textAlign: 'left' }}>平台</th>
                  <th style={{ padding: '8px', borderBottom: '1px solid #e8e8e8', textAlign: 'left' }}>详情</th>
                </tr>
              </thead>
              <tbody>
                  ...
                  <tr key={item.key} style={{ borderBottom: '1px solid #f0f0f0' }}>
                    ...
                    <td style={{ padding: '8px' }}>
                      <span style={{
                        ...
                        color: '#52c41a',
                        fontSize: '12px'
                      }}>{item.platform === 'web' ? '网页' : item.platform === 'mini-program' ? '小程序' : item.platform === 'nodejs' ? 'Node.js' : 'Electron'}</span>
                    </td>
